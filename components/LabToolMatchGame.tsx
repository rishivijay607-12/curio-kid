...
import { LAB_TOOLS } from '../constants.ts';

interface LabToolMatchGameProps {
  onEnd: () => void;
}

const GAME_DURATION = 60;

const LabToolMatchGame: React.FC<LabToolMatchGameProps> = ({ onEnd }) => {
  const [score, setScore] = useState(0);
  const [timeLeft, setTimeLeft] = useState(GAME_DURATION);
  const [currentItem, setCurrentItem] = useState<{ name: string; use: string } | null>(null);
  ...
  const setupNextQuestion = useCallback(() => {
    ...
    const correctItem = LAB_TOOLS[Math.floor(Math.random() * LAB_TOOLS.length)];
    setCurrentItem(correctItem);

    const wrongOptions: string[] = [];
    const itemsCopy = [...LAB_TOOLS];
    while (wrongOptions.length < 3) {
      const randomIndex = Math.floor(Math.random() * itemsCopy.length);
      const randomItem = itemsCopy.splice(randomIndex, 1)[0];
      if (randomItem.use !== correctItem.use) {
        wrongOptions.push(randomItem.use);
      }
    }

    setOptions(shuffleArray([correctItem.use, ...wrongOptions]));
  }, []);
  ...
  const getButtonClass = (option: string) => {
    ...
    if (option === currentItem?.use) return 'bg-green-700 border-green-500';
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-6 md:p-8 bg-slate-900 rounded-xl shadow-2xl border border-slate-800">
      ...
      <div className="text-center mb-8">
        <p className="text-slate-400 text-lg mb-2">What is the primary use of a:</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {options.map((option, index) => (
          <button
            ...
            disabled={isAnswered}
            className={`p-5 min-h-[90px] flex items-center justify-center rounded-lg border-2 text-center text-slate-100 font-semibold text-lg transition-all duration-300 ${getButtonClass(option)}`}
          >
            {option}
          </button>
        ))}
...
export default LabToolMatchGame;