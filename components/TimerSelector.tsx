...
interface TimerSelectorProps {
  onTimerSelect: (durationInSeconds: number) => void;
}
...
const TimerSelector: React.FC<TimerSelectorProps> = ({ onTimerSelect }) => {
  const options = [
    { label: 'No Timer', duration: 0 },
    { label: '30 Seconds', duration: 30 },
    { label: '1 Minute', duration: 60 },
    { label: '1.5 Minutes', duration: 90 },
  ];
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {options.map(({ label, duration }) => (
          <TimerButton key={label} label={label} onClick={() => onTimerSelect(duration)} />
        ))}
      </div>
    </div>
  );
